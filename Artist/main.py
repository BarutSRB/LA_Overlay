...
def setup_gui():
    ...
    for bar_name, pos in saved_positions.items():
        bars[bar_name] = DraggableRectangle(
            ...
            pos[0],
            pos[1],
            300 if bar_name == "Judgement" else 64,
            24 if bar_name == "Judgement" else 64,
            bar_name.lower(),
        )
...
def check_bars(positions):
    ...
    def check_bar(bar_pos, bar_length, image="./Assets/OCR/Orb.png"):
        try:
            if pyautogui.locateOnScreen(
                image,
                region=(int(bar_pos[0]), int(bar_pos[1]), bar_length, 70),
                confidence=0.8,
            ):
                return True
    ...
    if check_bar(positions["Bar1"], 70):
        gauge = 1
        if check_bar(positions["Bar2"], 70):
            gauge = 2
            if check_bar(positions["Bar3"], 70):
                gauge = 3
